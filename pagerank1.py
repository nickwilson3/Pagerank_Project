...
class WebGraph():
    ...
    def search(self, pi, query='', max_results=10, s_weight = 0.3, power = 30):
        '''
        ...
        for i in range(n):
            n1, score, weight = 0,0,0
            url1 = self._index_to_url(i)

            if url_satisfies_query(url1, query):
                n1 += 1
                weight += s_weight
            for word in range(10):
                w = S[word][0]

                if url_satisfies_query(url1, w):
                    n1 += 1
...
def url_satisfies_query(url, query):
    ...
    for term in terms:
        ...
        if term[0] != '-':
            num_terms+=1
            word_list = vectors.most_similar(term)
